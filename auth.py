"""
Simple Authentication System for AgentSDR
...
class AuthManager:
    ...
    def init_database(self):
        ...
        # Create CRM tokens table for OAuth integrations
        cursor.execute('''
        ...
        if cursor.fetchone()[0] == 0:
            sample_users = [
                ('super-admin-001', 'superadmin@agentsdr.com', 'Super Admin', 'AgentSDR', 'super_admin', 'active', self.hash_password('superadmin123')),
                ('admin-001', 'admin@agentsdr.com', 'Admin User', 'AgentSDR', 'admin', 'active', self.hash_password('admin123')),
                ('user-001', 'user@agentsdr.com', 'Regular User', 'AgentSDR', 'user', 'active', self.hash_password('user123')),
                ('user-002', 'pending@agentsdr.com', 'Pending User', 'AgentSDR', 'user', 'pending', self.hash_password('pending123'))
            ]
    ...
    def authenticate_user(self, email, password):
        """Authenticate user with email and password using SQLite"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, email, name, organization, role, status, password_hash, enterprise_id
                FROM users
            ...
            user_id, user_email, name, organization, role, status, password_hash, enterprise_id = user
    ...
    def register_user(self, email, password, name, organization, role='user', status='active', enterprise_id=None):
        """Register new user and return user data using SQLite"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO users (id, email, name, organization, role, status, password_hash, enterprise_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, email, name, organization, role, status, password_hash, enterprise_id))
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Also set in Flask's g object for easy access
        g.user_id = user_data.get('user_id')  # JWT uses 'user_id' not 'id'
