...
def redirect_non_www():
    host = request.host
    # Domain redirect logic can be added here if needed
    pass
...
def load_user_context():
    ...
    if hasattr(request, 'current_user') and request.current_user:
        g.user_id = request.current_user.get('id')
        g.current_user = request.current_user
        print(f"🔍 User context loaded: {g.user_id}")
    else:
        g.user_id = None
        g.current_user = None
...
# Register CRM integration blueprint
try:
    from crm_routes import crm_bp
    app.register_blueprint(crm_bp)
    print("✅ CRM integration routes registered")
except ImportError as e:
    print(f"⚠️  CRM integration not available: {e}")
...
def load_enterprise_context():
    ...
    try:
        # Check if Supabase is available, if not use SQLite
        if not SUPABASE_AVAILABLE:
            # Use SQLite to get user's enterprise_id
            import sqlite3
        ...
        # Get user's enterprise_id from Supabase
...
def get_voice_agents():
    ...
    try:
        ...
        # Check if Supabase is available, if not use SQLite
        if not SUPABASE_AVAILABLE:
            # Use SQLite to get voice agents
            import sqlite3
            conn = sqlite3.connect('users.db')
            cursor = conn.cursor()
            ...
            cursor.execute("""
                SELECT id, name, language, use_case, calling_number, status,
                       created_by, enterprise_id, configuration, created_at, updated_at
                FROM voice_agents
                WHERE enterprise_id = ?
        ...
        # Get voice agents for the enterprise from Supabase
...
def manual_call():
    ...
    try:
        ...
        variables = {
            ...
            'greeting': 'Hello, this is a call from AgentSDR. How can I assist you today?',
...
def purchase_phone_number_production():
    ...
    try:
        ...
        phone_number = data.get('phone_number')
        provider_name = data.get('provider')
        friendly_name = data.get('friendly_name', f"AgentSDR - {phone_number}")
...
def handle_voice_webhook():
    ...
    try:
        ...
        <Response>
            <Say>Welcome to AgentSDR. Please wait while we connect you to our AI voice agent.</Say>
...
# Duplicate health_check removed - using comprehensive one above
...
def get_dashboard_stats():
    ...
    try:
        ...
        # For now, provide realistic mock data based on user role
        # In production, this would query real data from Supabase
        ...
        if user_role == 'super_admin':
            ...
            stats = {
                ...
                'phone_numbers': 23,
                'inbound_channels': 28,
                'outbound_channels': 12,
                'whatsapp_channels': 5,
                'active_contacts': 2456,
                'contacted_today': 156,
                'response_rate': 87.3,
                'countries': 8,
                'active_providers': 3
            ...
            # Admin sees enterprise-wide stats
            stats = {
                'organizations': 3,
                'channels': 12,
                'voice_agents': 18,
            ...
            # Regular user sees their organization stats
            stats = {
                'organizations': 1,
                'channels': 4,
                'voice_agents': 6,
...
@login_required
def get_admin_users():
    """Get all users across all enterprises for superadmin management"""
    try:
...
def get_admin_organizations():
    """Get all organizations across all enterprises for superadmin management"""
    try:
        # Check if user is super_admin
        current_user = request.current_user
        if not current_user or current_user.get('role') != 'super_admin':
        ...
        organizations = supabase_request('GET', 'organizations') or []
...
@app.route('/')
def serve_landing():
    """Serve the new animated robot landing page"""
    return send_from_directory(app.static_folder, 'index.html')

@app.route('/old-landing')
def serve_old_landing():
...
if __name__ == "__main__":
    ...
    print("Starting AgentSDR SaaS Platform")
